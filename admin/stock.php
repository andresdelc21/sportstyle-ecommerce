...
<section class="admin-hero small-hero"><div><span class="admin-badge">Inventario</span><h1>Gestión de stock</h1><p>Actualizá cantidades rápidamente y detectá faltantes.</p></div></section>
...
<form method="POST">
<div class="tabla-admin tabla-premium"><table><thead><tr><th>Producto</th><th>Precio</th><th>Stock</th><th>Estado</th></tr></thead><tbody>
<?php while($p=mysqli_fetch_assoc($productos)): ?><tr><td><div class="producto-admin-info"><img src="../<?= htmlspecialchars($p['imagen']) ?>" class="admin-img-producto"><strong><?= htmlspecialchars($p['nombre']) ?></strong></div></td><td>$<?= number_format($p['precio'],0,',','.') ?></td><td><input class="stock-input-admin" type="number" min="0" name="stock[<?= $p['id'] ?>]" value="<?= (int)$p['stock'] ?>"></td><td><?php if($p['stock']<=0): ?><span class="stock-bajo">Sin stock</span><?php elseif($p['stock']<=5): ?><span class="estado pendiente">Stock bajo</span><?php else: ?><span class="stock-ok">Disponible</span><?php endif; ?></td></tr><?php endwhile; ?>
</tbody></table></div>
