...
$editando = null;
if(isset($_GET['toggle'])){ $id=(int)$_GET['toggle']; mysqli_query($conn,"UPDATE banners SET activo=IF(activo=1,0,1) WHERE id=$id"); header("Location: banners.php"); exit; }
if(isset($_GET['eliminar'])){ $id=(int)$_GET['eliminar']; mysqli_query($conn,"DELETE FROM banners WHERE id=$id"); header("Location: banners.php"); exit; }
if(isset($_GET['editar'])){ $id=(int)$_GET['editar']; $res=mysqli_query($conn,"SELECT * FROM banners WHERE id=$id"); $editando=mysqli_fetch_assoc($res); }
...
<section class="pedido-panel"><form method="POST" class="form-admin-premium"><input type="hidden" name="id" value="<?= (int)($editando['id']??0) ?>"><div class="admin-grid">
